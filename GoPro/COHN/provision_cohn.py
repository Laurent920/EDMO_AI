...
from GoPro import GoProUuid, connect_ble, proto, connect_to_access_point, ResponseManager, logger
...
async def main(ssid: str, password: str, identifier: str | None, certificate: Path) -> Credentials | None:
    ...
    try:
        ...
        # Create the folder and files if not existing
        print(cwd)
        certificate = cwd / certificate
        print(certificate)
        certificate.parent.mkdir(parents=True, exist_ok=True)
        certificate.touch(exist_ok=True)
        
        credPath = cwd / Path(*certificate.parts[:-1]) / Path('credentials.txt')
...
if __name__ == "__main__":
    ...
    try:
        ...
        cwd = os.getcwd()
        asyncio.run(main(args.ssid, args.password, identifier, Path(f'{cwd}/GoPro/{identifier}/cohn.crt')))
